    ...
    dashboard["panels"] = [
        ...
        table_panel(
            ...
            sql=(
                ...
                "    END AS explanation, "
                "    CASE COALESCE(driver.value->>'key', '') "
                "        WHEN 'trend_score' THEN '67-100 strong uptrend bias' "
                "        WHEN 'momentum_score' THEN '67-100 strong upside acceleration' "
                "        WHEN 'relative_strength_score' THEN '67-100 leading peer performance' "
                "        WHEN 'rsi' THEN '45-60 healthy momentum zone' "
                "        WHEN 'volume_ratio' THEN '>=1.5 strong participation' "
                "        WHEN 'atr_pct' THEN '<2 controlled daily volatility' "
                "        ELSE '' "
                "    END AS positive_range, "
                "    CASE COALESCE(driver.value->>'key', '') "
                "        WHEN 'trend_score' THEN '40-66 mixed trend quality' "
                "        WHEN 'momentum_score' THEN '40-66 modest price follow-through' "
                "        WHEN 'relative_strength_score' THEN '40-66 average peer strength' "
                "        WHEN 'rsi' THEN '35-44 or 61-70 watch extremes' "
                "        WHEN 'volume_ratio' THEN '0.8-1.49 normal activity' "
                "        WHEN 'atr_pct' THEN '2-4 normal daily movement' "
                "        ELSE '' "
                "    END AS neutral_range, "
                "    CASE COALESCE(driver.value->>'key', '') "
                "        WHEN 'trend_score' THEN '0-39 weak or broken trend' "
                "        WHEN 'momentum_score' THEN '0-39 fading price impulse' "
                "        WHEN 'relative_strength_score' THEN '0-39 lagging peer performance' "
                "        WHEN 'rsi' THEN '<35 or >70 reversal risk' "
                "        WHEN 'volume_ratio' THEN '<0.8 weak conviction' "
                "        WHEN 'atr_pct' THEN '>4 unstable daily swings' "
                "        ELSE '' "
                "    END AS negative_range, "
